...
import static com.pws.admin.entity.User.SEQUENCE_NAME;
...
public class AdminServiceImpl implements AdminService {
    ...
    public void addRole(Role role) throws PWSException {
        role.setId(service.getSequenceNumber(SEQUENCE_NAME));
    ...
    public void addModule(Module module) throws PWSException {
        module.setId(service.getSequenceNumber(SEQUENCE_NAME));
    ...
    public void saveOrUpdateUserXref(UserRoleXrefDTO userRoleXrefDTO) throws PWSException {
        ...
        if (optionalUserRoleXref.isPresent()) {
            ...
            userRoleXref.setId(service.getSequenceNumber(SEQUENCE_NAME));
        }
    ...
    public void addPermission(PermissionDTO permissionDTO) throws PWSException {
        ...
        permission.setId(service.getSequenceNumber(SEQUENCE_NAME));
        ...
        Optional<Module> module = moduleRepository.findById(permissionDTO.getModuleId());
        permission.setModule((Module) module.get());
        Optional<Role> role = roleRepository.findById(permissionDTO.getRoleId());
        permission.setRole((Role) role.get());
    ...
    public void updatePermission(PermissionDTO permissionDTO) throws PWSException {
        ...
        if (optionalpermission.isPresent()) {
            ...
            Optional<Module> module = moduleRepository.findById(permissionDTO.getModuleId());
            optionalpermission.get().setModule((Module) module.get());
            Optional<Role> role = roleRepository.findById(permissionDTO.getRoleId());
            optionalpermission.get().setRole((Role) role.get());
    ...
    public UserBasicDetailsDTO getUserBasicInfoAfterLoginSuccess(String email) throws PWSException {
        ...
        List<Role> roleList = userRoleXrefRepository.fetchAllUsersByRole(user.getId());
        userBasicDetailsDTO.setRoleList(roleList);
        List<Permission> permissionList = null;
        if (roleList.size() > 0)
            permissionList = permissionRepository.getPermissionsByRoleId(roleList.get(0).getId());
    ...
    public void addskill(Skill skill) throws PWSException {
        skill.setId(service.getSequenceNumber(SEQUENCE_NAME));
